...
app.post('/api/register', (req, res) => {
    const { nom, prenom, ddn, nomclass, numCartEtu, login, password } = req.body;
    console.log(password);
    const hashedPassword = bcrypt.hashSync(password, 10);
    console.log(hashedPassword);
    const sql = 'INSERT INTO etudiant (nom, prenom,ddn,nomclass,numCartEtu,login,password) VALUES (?,?,?,?,?,?,?)';
    pool.query(sql, [nom, prenom, ddn, nomclass, numCartEtu, login, hashedPassword], (err, result) => {
        if (err) {
